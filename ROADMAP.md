# Roadmap

This roadmap lists planned work after v0.1.0.

## v0.2+

- Cluster / remote execution support.
- Optional email notifications for completion and failure.
- Extended diagnostics and richer run summaries.

## v0.3

- Remote control workflows for managed long-running experiments.
- Broader execution environment integrations beyond local Docker-first flow.
- TUI for interactive run monitoring and control.